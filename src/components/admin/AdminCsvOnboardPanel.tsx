    ...
    <div className="space-y-4">
      <div className="flex items-baseline gap-3 flex-wrap">
        ...
        <span className="text-[12px] ml-1 hidden md:inline" style={{ color: "#7A8290" }}>
          · {preview ? `${preview.ok} ready` : "paste to start"}
        </span>
      </div>
                ...
                {preview.rows.map((r) => {
                  ...
                  return (
                    <tr key={r.rowIdx} style={{ borderTop: "1px solid #e8e5e0", background: ok ? "white" : "rgba(231,0,19,0.04)" }}>
                      <td className="px-2 py-2 font-mono text-[11px]" style={{ color: "rgba(0,0,0,0.55)" }}>{r.rowIdx}</td>
                      <td className="px-2 py-2 font-bold" style={{ color: NOHO_INK }}>{r.name || <span style={{ color: "rgba(0,0,0,0.40)" }}>—</span>}</td>
                      <td className="px-2 py-2" style={{ color: NOHO_INK }}>{r.email || <span style={{ color: "rgba(0,0,0,0.40)" }}>—</span>}</td>
                      <td className="px-2 py-2 font-mono">{r.suiteNumber ?? <span style={{ color: "rgba(0,0,0,0.40)" }}>—</span>}</td>
                      <td className="px-2 py-2">{r.plan ?? <span style={{ color: "rgba(0,0,0,0.40)" }}>—</span>}</td>
          ...
          {preview.ok > 0 && (
            !confirming ? (
              ...
              <div className="rounded-xl border-2 p-3 space-y-2" style={{ borderColor: NOHO_RED, background: "rgba(231,0,19,0.04)" }}>
                ...
                <p className="text-[10.5px]" style={{ color: "rgba(0,0,0,0.65)" }}>
