    ...
    <div className="space-y-4">
      <div className="flex items-baseline gap-3 flex-wrap">
        ...
        <span className="text-[12px] ml-1 hidden md:inline" style={{ color: "#7A8290" }}>
          · {codes ? `${codes.length} active${stale.length > 0 ? ` · ${stale.length} stale` : ""}` : "loading"}
        </span>
      </div>
      <p className="text-[11px]" style={{ color: T.inkFaint }}>
        One active 4-digit code per customer. Default rotation 90 days. Every keypad attempt logged.
      </p>
